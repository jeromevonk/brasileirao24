    ...
    for item in data:
        try:
            ...
            round.append({
                ...
                "date": item["data_realizacao"][:10] if isinstance(item["data_realizacao"], str) else None,
                "started" : item["jogo_ja_comecou"]
            })
        except Exception as e:
            print(f'Something wrong with match between {item["equipes"]["mandante"]["nome_popular"]} and {item["equipes"]["visitante"]["nome_popular"]}')
...
def run(_event, _context):
    ...
    try:
        for i in range(1, 39):
            ...
            r = requests.get(f'https://api.globoesporte.globo.com/tabela/d1a37fa4-e948-43a6-ba53-ab24ab3a45b1/fase/fase-unica-campeonato-brasileiro-2024/rodada/{i}/jogos/', verify=False)
            data = r.json()
